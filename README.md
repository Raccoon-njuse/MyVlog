...
- 用户上传的视频完成转码后直接进入已公开状态；管理员可删除视频，并处理 LRC 歌词更新后产生的 Relink 暂存任务。
...
- `/uploader`：上传者页面采用移动端优先界面。用户直接输入姓名登录；页面仍以歌词为主，自己上传过的视频会在对应歌词行显示“我的”标注，点击歌词行后在行内展开该句下自己的视频。顶栏提供音乐模式和“添加视频”；“添加视频”会先进入歌词选择模式，每行前出现选框且整行可切换选择；此时顶栏按钮变为“下一步”，右侧 `×` 用于取消本次选择。未选择歌词时也可继续上传，视频会作为只绑定参与者的花絮保存。
- `/admin`：管理员后台。只有输入 `raccoon` 才会加载后台数据；后台不显示侧边导航，只保留退出登录、节拍器、歌词进度、视频删除和 Relink 处理。节拍器完全运行在浏览器端，不写入数据库；管理员不提供音乐模式，也不提供歌词逐句编辑或结构编辑。

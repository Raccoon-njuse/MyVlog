...
- 当前歌词不再由管理员在页面维护，服务启动和 LRC 文件变更时会读取 `storage/audios/中文版.lrc` 与 `storage/audios/英文版.lrc`，按中文版在前、英文版在后的顺序整体同步。
- 上传视频会保留原始文件，并同步生成低码率 MP4 播放副本；前端只播放低码率副本，未转码完成的视频不会挂载播放器。
- 访客页不需要登录，歌词行覆盖数量会统计已公开、已激活的可播放素材；访客接口只返回覆盖数量，不下发视频播放地址。
- 上传者页使用姓名登录，不需要密码；登录会自动注册姓名。上传者可上传素材并查看自己名下的视频状态，管理员开启预览权限后可在歌词行内查看所有用户已公开的可播放素材。
- 管理员页使用姓名登录，不需要密码；管理员姓名固定为 `raccoon`。
- 用户上传的视频完成转码后直接进入已公开状态；管理员可删除视频，管理用户预览权限，并处理 LRC 歌词更新后产生的 Relink 暂存任务。
- 访客页和上传者页顶栏提供音乐模式。进入后只能纵向浏览歌词，点击某句会从 LRC 记录的时间点播放对应版本音频，再次点击音乐按钮会退出并停止播放。
- 管理员后台内置纯前端节拍器，可设置 BPM、每小节拍数、敲击定速并播放重拍提示音，便于录制前统一速度。
...
- `/`：访客总览采用移动端优先界面。页面以歌词行列表为主，每行右侧只显示覆盖数量，`0` 会以红色提示；顶栏保留音乐模式入口和“登录”，普通姓名进入上传者页面，`raccoon` 进入管理员页面，访客页不提供添加按钮。
- `/uploader`：上传者页面采用移动端优先界面。用户直接输入姓名登录，姓名会写入 `persons` 作为注册用户；页面仍以歌词为主，自己上传过的视频会在对应歌词行显示“我的”标注。默认点击歌词行后只在行内展开该句下自己的视频；如果管理员为该姓名开启预览权限，则展开时显示该句下所有用户已公开且可播放的视频。顶栏提供音乐模式和“添加视频”；“添加视频”会先进入歌词选择模式，每行前出现选框且整行可切换选择；此时顶栏按钮变为“下一步”，右侧 `×` 用于取消本次选择。未选择歌词时也可继续上传，视频会作为只绑定参与者的花絮保存。
- `/admin`：管理员后台。只有输入 `raccoon` 才会加载后台数据；后台不显示侧边导航，只保留退出登录、节拍器、用户管理、歌词进度、视频删除和 Relink 处理。用户管理模块展示所有已注册姓名，并用开关控制上传者页面是否可以预览所有用户已公开素材。节拍器完全运行在浏览器端，不写入数据库；管理员不提供音乐模式，也不提供歌词逐句编辑或结构编辑。
...
当前登录只是轻量姓名识别，不做密码、会话签名或服务端持久 session。前端会把姓名保存在浏览器 `localStorage`，请求需要识别身份的接口时通过 URL 编码后的 `X-User-Name` 请求头传给后端，以支持中文姓名。后端会用姓名查找或创建 `persons` 记录，并通过 `persons.can_preview_all_videos` 控制上传者页面是否返回全员素材预览数据。
...
- `006_source_lyrics_from_lrc.sql`：为歌词增加 LRC 来源、行号和音频时间点字段，并新增 LRC 来源版本记录表。
- `007_add_user_preview_permission.sql`：为用户增加全员素材预览权限字段和姓名查询索引。
...
当前固定读取 `中文版.lrc`、`中文版.mp3`、`英文版.lrc`、`英文版.mp3`。服务启动时会先解析两个 LRC 文件并同步到 `lyric_units`；服务运行中监听到任意 `.lrc` 文件更新，也会重新同步。只要 LRC 指纹变化，现有激活的视频歌词关联都会进入 Relink 暂存区，等待管理员重新选择当前歌词句子。
